...
class ANN_alpha1(nn.Module):
    def __init__(self,input_dim,num_classes):
        super().__init__()
        self.input_dim=input_dim
        self.num_classes=num_classes
        self.ff1=nn.Linear(input_dim,4096)
        self.ff2=nn.Linear(4096,2048)
        self.ff3=nn.Linear(2048,1024)
        self.ff4=nn.Linear(1024,512)
        self.ff5=nn.Linear(512,256)
        self.ff6=nn.Linear(256,128)
        self.ff7=nn.Linear(128,64)
        self.ff8=nn.Linear(64,1)
        self.ff9=nn.Linear(64,num_classes)
...
def main():
    ...
    alpha1_model = ANN_alpha1(input_dim=33011, num_classes=1)
    alpha2_model = ANN_alpha1(input_dim=2048, num_classes=1)
    alpha1_model = alpha1_model.cuda()
    alpha2_model = alpha2_model.cuda()
    ...
    def train_step(
        ...
        with torch.no_grad():
            # do the full forward pass for the teacher model (decoder)
            teacher_outputs = teacher_model(**batch)
        alpha1 = alpha1_model(torch.cat((teacher_outputs.logits, student_outputs.logits), 1)).squeeze()
        alpha2 = alpha2_model(torch.cat((teacher_outputs.hidden_states[-1], teacher_outputs.hidden_states[-1]), 1)).squeeze()
        alpha1 = torch.max(alpha1).item()
        alpha2 = torch.max(alpha2).item()
        ...
        if model_args.loss_type=='KD_loss':
        ...
        elif model_args.loss_type=='T_S_KD_loss':
            ...
            kl_loss_S = (
                unsupervised_kl_divergence(teacher_hidden_state, student_hidden_state, batch["labels"], alpha2)
            )
            logit_teacher = teacher_outputs.logits
            logit_student = student_outputs.logits
            kl_loss = alpha1 * kl_loss_ST + (1-alpha1) * kl_loss_TS
            loss = (1-(alpha1+alpha2)) * ce_loss + kl_loss + kl_loss_S
        
        elif model_args.loss_type=='U_KD_loss':
            teacher_distribution = teacher_outputs.hidden_states[-1]
            student_distribution = student_outputs.hidden_states[-1]
            kl_loss = (
                unsupervised_kl_divergence(teacher_distribution, student_distribution, batch["labels"])
            )
            loss = 0.5 * ce_loss + training_args.kl_weight * kl_loss
        metrics = {"loss": loss, "ce_loss": ce_loss, "kl_loss": kl_loss, 'alpha1': alpha1, 'alpha2': alpha2}
        return loss, metrics
            ...
            if accelerator.sync_gradients:
                ...
                if cur_step % training_args.logging_steps == 0:
                    steps_trained_progress_bar.write(
                        ...
                        f" {train_metric['loss'].item(), train_metric['kl_loss'].item(), train_metric['ce_loss'].item(), train_metric['alpha1'], train_metric['alpha2']}, Learning Rate:"
