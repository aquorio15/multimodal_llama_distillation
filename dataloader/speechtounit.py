...
if __name__ == '__main__':
    ...
    ckpt_dir = "/DATA/nfsshare/Amartya/NAACL"

    #path = get_files_from_path('/DATA/nfsshare/Amartya/NAACL/shrutilipi/LibriLight/large_new', '.wav')
    with open('/DATA/nfsshare/Amartya/NAACL/shrutilipi/Librispeech1.txt', 'r') as h:
        paths = h.readlines()
    s2u = Speech2Unit(
        ckpt_dir=ckpt_dir
    )
    #/data/nfsshare/Amartya/NAACL/shrutilipi/LibriSpeech/train-clean-100/19/198/19-198-0000.flac
    with open('/DATA/nfsshare/Amartya/NAACL/shrutilipi/libri_units1.txt', 'w') as f:
        for path in tqdm.tqdm(paths):
            file_path = path.strip().split('\t')[0]
            file_path = '/DATA'+file_path
            text = path.strip().split('\t')[-1]
            try:
                name = os.path.basename(file_path).split('.')
                units = s2u(file_path)
                #units = s2u.run(n_procs=100)
                f.write(str(units)+'\t'+str(text)+'\n')
            except:
                continue
# %%
